...
import { FailedJob, PrintJobStatus } from '../types';
...
export class FailedJobService {
  ...
  public async getFailedJobs(): Promise<FailedJob[]> {
    return await this.failRepo.findAll();
  }

  public async getFailedJobById(id: string): Promise<FailedJob | null> {
   ...
   */
  public async retryFailedJob(failedId: string): Promise<{ success: boolean; newJobId?: string; error?: string }> {
  ...
  public async purgeFailedJob(failedId: string): Promise<boolean> {
