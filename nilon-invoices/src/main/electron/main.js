...
function registerIpcHandlers() {
    ...
    ipcMain.handle(IPC_CHANNELS.PRINTER.GET_LIST, () => {
        return db.prepare('SELECT * FROM printers ORDER BY is_default DESC, created_at ASC').all();
    });
    ...
    ipcMain.handle(IPC_CHANNELS.PRINTER.ADD, (_event, printerPayload) => {
    ...
    ipcMain.handle(IPC_CHANNELS.PRINTER.UPDATE, (_event, printer) => {
        printerManager.updatePrinter(printer);
        return { success: true };
    });
    ipcMain.handle(IPC_CHANNELS.PRINTER.DELETE, (_event, id) => {
        printerManager.deletePrinter(id);
        return { success: true };
    });
    ipcMain.handle(IPC_CHANNELS.PRINTER.SET_DEFAULT, (_event, id) => {
        printerManager.updatePrinter({ id, is_default: 1 });
        return { success: true };
    });
    ipcMain.handle(IPC_CHANNELS.PRINTER.TEST, async (_event, id) => {
        try {
    ...
    ipcMain.handle(IPC_CHANNELS.JOB.GET_ACTIVE, () => {
        return db.prepare("SELECT * FROM print_jobs WHERE status IN ('PENDING', 'PRINTING') ORDER BY created_at ASC").all();
    });
    ipcMain.handle(IPC_CHANNELS.JOB.GET_HISTORY, () => {
        return db.prepare("SELECT * FROM print_jobs WHERE status IN ('SUCCESS', 'FAILED') ORDER BY created_at DESC LIMIT 100").all();
    });
    ipcMain.handle(IPC_CHANNELS.JOB.REPRINT, (_event, jobId) => {
        const job = db.prepare('SELECT * FROM print_jobs WHERE id = ?').get(jobId);
        if (!job)
            throw new Error('Target print job not found in history cache.');
    ...
    ipcMain.handle(IPC_CHANNELS.JOB.CANCEL, (_event, jobId) => {
    ...
    ipcMain.handle(IPC_CHANNELS.SETTINGS.UPDATE, (_event, settingsPayload) => {
        db.transaction(() => {
    ...
    ipcMain.handle(IPC_CHANNELS.SETTINGS.SET_STARTUP, (_event, shouldRunOnStartup) => {
    ...
    ipcMain.handle(IPC_CHANNELS.SYSTEM.GET_LOGS, () => {
        return db.prepare('SELECT * FROM printer_logs ORDER BY timestamp DESC LIMIT 200').all();
    });
