...
class PrinterManager {
    ...
    routeToQueue(job) {
        ...
        if (!this.queues.has(job.printer_id)) {
            ...
            if (printer) {
                ...
                const defaultPrinter = db.prepare('SELECT * FROM printers WHERE is_default = 1').get();
                if (defaultPrinter) {
    ...
    async printTestPage(printerId) {
        ...
        if (!printer)
            throw new Error('Thermal printer does not exist in SQLite registry.');
    ...
    addPrinter(printer) {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(printer.id, printer.name, printer.connection_type, printer.ip_address, printer.port, printer.paper_size, printer.is_default, printer.status);
        })();
