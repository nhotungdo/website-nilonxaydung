...
export function registerDatabaseIpcHandlers() {
    ...
    // 2. createOrder()
    ipcMain.handle('db:create-order', async (_event, orderDto) => {
    ...
    // 4. addPrinter()
    ipcMain.handle('db:add-printer', async (_event, printerDto) => {
        logger.ipc('db:add-printer', printerDto);
        try {
            if (!printerDto)
                throw new Error('Printer payload is required.');
    ...
    // 6. retryFailedJob()
    ipcMain.handle('db:retry-failed-job', async (_event, failedId) => {
        logger.ipc('db:retry-failed-job', { failedId });
        try {
            if (!failedId)
                throw new Error('Failed job troubleshooting ID is required.');
    ...
    // 8. saveSettings()
    ipcMain.handle('db:save-settings', async (_event, settingsDto) => {
