...
export const GlassCard: React.FC<GlassCardProps> = ({
  ...
  return (
    <div
      onClick={onClick}
      className={`relative rounded-2xl border bg-gradient-to-br from-white/[0.04] to-transparent p-5 backdrop-blur-md transition-all duration-300 ${glowClasses[glowColor]} ${interactiveClasses} ${className}`}
    >
      {children}
    </div>
  );
